...
def format_wikipedia(s):
    ...
    for line in s.split('\n'):
        ...
        for m in rx_u.findall(line):
            ...
            replacements.append(
                (m, '<a href="/c/{}">U+{}</a>'.format(code, code)))
...
class UInfo:
        ...
        with open(file_name, encoding='utf-8') as f:
            ...
            for line in f:
                if re.match(r'^[0-9A-F]{4,6}\t', line):
                    ...
                    subblock = None
                    m = re.match(r'^@@\t([0-9A-F]{4,6})\t(.*)\t([0-9A-F]{4,6})$', line)
    ...
    def _load_confusables(self, file_name):
        ...
        with open(file_name, encoding='utf-8') as f:
            rx = re.compile(r'^\s*([0-9A-Fa-f]{4,6})\s*;\s*([0-9A-Fa-f]{4,6})\s*;\s*MA')
            sets = {}
            for line in f:
                line = line.strip()
                if line.startswith('#') or line == '':
    ...
    def _load_casefolding(self, file_name):
        ...
        with open(file_name, encoding='utf-8') as f:
            rx = re.compile(r'^\s*([0-9A-Fa-f]{4,6}); C; ([0-9A-Fa-f]{4,6}); #')
            for line in f:
                line = line.strip()
                if line.startswith('#') or line == '':
                    continue
    ...
    def _load_unihan(self, file_name):
        ...
        with open(file_name, 'r', encoding='utf-8') as f:
            rx = re.compile(r'^U\+([0-9A-Fa-f]{4,6})\tkDefinition\t(.*)$')
            for line in f:
                line = line.strip()
                m = rx.match(line)
                if m:
    ...
    def _load_hangul(self, file_name):
        ...
        with open(file_name, 'r', encoding='utf-8') as f:
            ...
            rx = re.compile(r'^\s*[0-9]+\s*0x([0-9A-Fa-f]{4,6})\s+.*\((.+)\)\s*$')
            for line in f:
                line = line.strip()
                m = rx.match(line)
                if m is None:
    ...
    def search_by_name(self, keyword, limit):
        ...
        for k in keyword.upper().split():
            ...
            if k != '':
                keywords.append(k)
        if len(keywords) == 1:
            m = re.match(r'^U?\+?([0-9A-F]{1,6})$', keywords[0], re.IGNORECASE)
            if m:
        ...
        deprioritized_blocks = [
            0x2E80, 0x2F00, 0x31C0, 0x3300, 0x3400, 0x4E00, 0xF900, 0x20000, 0x2A700, 0x2B740, 0x2B820, 0x2F800]
        limit_reached = False
